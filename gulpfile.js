...
gulp.task('update', ['makeZip', 'lint'], function update() {
  ...
  .pipe(lambda('FavouritePlaces', { region: 'eu-west-1' }));
});

gulp.task('create', ['makeZip', 'lint'], function create() {
  return gulp.src('./build/package/lambda.zip')
  .pipe(lambda(params, { region: 'eu-west-1' }))
  .pipe(shell('aws lambda add-permission --function-name FavouritePlaces --statement-id "1234" --action "lambda:InvokeFunction" --principal "alexa-appkit.amazon.com"  --region eu-west-1'));
});
